...
public:
    Card() {
        ...
        face = faces[rand()%15]; //randomly chosen face
    ...
    void printCard1() {
        std::cout << color.getColor() << " " << face.getFace() << " (" << getValue() << " points) " << std::endl; //displays each card by their color and face
    }
    void printCard() {
...
public:
    ...
    int highestNumberCard() {
        int highest = -1;
        for (unsigned int i = 0; i < cards.size(); i++) {
            if (cards[i].getValue() <= 9 and cards[i].getValue() > highest) {
                highest = cards[i].getValue();
            }
        }
        return highest;
    }
    unsigned int getLowestCard() {
        int v = 51;
        std::vector<Card> low;
        std::vector<Card> lowest;
        for (unsigned int i = 0; i < cards.size(); i++) {
            if (cards[i].getValue() < v) {
                v = cards[i].getValue();
                low.push_back(cards[i]);
            }
        }
        if (v < 50) {
            std::string color = "";
            for (unsigned int i = 0; i < low.size(); i++) {
                if (colorExists(Color("red")) == 0) {
                    color = "red";
                } else if (colorExists(Color("yellow")) == 0) {
                    color = "yellow";
                } else if (colorExists(Color("green")) == 0) {
                    color = "green";
                } else {
                    color = "blue";
                }
            ...
            for (unsigned int i = 0; i < low.size(); i++) {
                if (low[i].getValue() == v and low[i].getColorCard().getColor().compare(color) == 0) {
                    lowest.push_back(low[i]);
                }
            }
            return rand()%(lowest.size());
        }
        return rand()%(low.size());
    }
    void removeLowest(int c) {
        std::vector<Card> a;
...
int positionToRemoveCard(Player a, std::vector<int> play, Player b) {
    std::vector<int> posDraw4; //position of draw4s
    std::vector<int> posWild; //position of wilds
    std::vector<int> otherCards; //position of other cards
    std::vector<int> draw2;
    std::vector<int> skips;
    std::vector<int> numbers;
    if (b.getDeck().getNumberOfCards() >= 4) { //if opponent's deck has 4 or more cards
        for (unsigned int i = 0; i < play.size(); i++) {
            if (a.getDeck().getCardAtPos(play[i]-1).getFaceCard().getFace().compare("DRAW4WILD") == 0) {
                posDraw4.push_back(play[i]);
            } else if (a.getDeck().getCardAtPos(play[i]-1).getFaceCard().getFace().compare("WILD") == 0) {
                posWild.push_back(play[i]);
            } else {
                otherCards.push_back(play[i]);
            }
        }
        if (otherCards.size() != 0) {
            return otherCards[rand()%(otherCards.size())];
    ...
    for (unsigned int i = 0; i < play.size(); i++) {
        if (a.getDeck().getCardAtPos(play[i]-1).getFaceCard().getFace().compare("DRAW4WILD") == 0) {
            ...
            posWild.push_back(play[i]);
        } else if (a.getDeck().getCardAtPos(play[i]-1).getFaceCard().getFace().compare("DRAW2") == 0){
            draw2.push_back(play[i]);
        } else if (a.getDeck().getCardAtPos(play[i]-1).getFaceCard().getFace().compare("REVERSE") == 0 or a.getDeck().getCardAtPos(play[i]-1).getFaceCard().getFace().compare("SKIP") == 0) {
...
Color chooseColor(Player a, Card start) {
...
void displayEffect(Player a, int token, Card start, std::string playWith) {
...
int main(int argc, char** argv) {
    ...
    while (playedAgain == true) {
        ...
        } else if (startingCard.getFaceCard().getFace().compare("WILD") == 0 or startingCard.getFaceCard().getFace().compare("DRAW4WILD") == 0) {
        ...
        std::vector<Card> playableCards3 = getPlayableCards(p3.getDeck(),startingCard); //playable cards for player 2
        std::vector<int> positions3 = getPlayablePositions(p3.getDeck(),startingCard);
        std::vector<Card> playableCards4 = getPlayableCards(p4.getDeck(),startingCard); //playable cards for player 2
